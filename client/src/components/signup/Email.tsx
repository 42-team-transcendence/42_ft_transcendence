...
interface EmailProps {
	...
	fonctionUpdateEmail: {
	  ...
	  updateValidEmail: (isValid: boolean) => void;
	};
  }

function Email ({ stateEmail, fonctionUpdateEmail }: EmailProps) {

    const {email, validEmail} = stateEmail;
    const {updateEmail, updateValidEmail} = fonctionUpdateEmail;

    useEffect(() => {
        updateValidEmail(EMAIL_REGEX.test(email));
    }, [email])
