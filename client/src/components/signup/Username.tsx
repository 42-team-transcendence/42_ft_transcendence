...
interface UsernameProps {
	...
	fonctionUpdate: {
	  updateUser: (newValue: string) => void;
	  updateValidName: (isValid: boolean) => void;
	};
  }

function Username ({ stateUser, fonctionUpdate }: UsernameProps) {

   const {user, validName} = stateUser;
   const {updateUser, updateValidName} = fonctionUpdate;
    
    useEffect(() => {
        updateValidName(USER_REGEX.test(user));
    }, [user])
