...
import { ChannelInfo, ChannelMutedUsers, Chat, User, } from '@prisma/client'; //User Typescript type generated by Prisma
import { AllChannelInfo, AllChatInfo, ChatAndParticipants, MutedUsersAndChannelInfo } from "./chat";
...
// export class ChatService implements IChatService {
	...
	async createChannel(payload: CreateChannelDto, creatorId: number): Promise<Chat> {
		try {
	...
	async getChannelsByName(input: string): Promise<AllChatInfo[]> {
		try {
			...
			return channels as AllChatInfo[];
		} catch (error) {
            console.log(error);
            throw error;
		}
	...
	async checkIsValidSender(msg: MessageDto): Promise<boolean> {
		try {
  	...
  	async checkIsAdmin(chatId:number, userId: number): Promise<boolean> {
		try {
			//Récupération des infos du chat
	...
	async deleteChannelMutedUser(
		channelInfoId: number,
		userId:number
	): Promise<MutedUsersAndChannelInfo> {
		try {
	...
	async upsertChannelMutedUsers(chatId: number, userId:number, payload:any): Promise<{ chat: {participants : User[]} } & AllChannelInfo> {
